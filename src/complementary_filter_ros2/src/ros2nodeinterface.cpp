...
// C/C++ source code generated on : Mon Jun 17 12:49:18 2024
//
#ifdef _MSC_VER
#pragma warning(push)
...
#include <chrono>
#include <utility>
const std::string SLROSNodeName("complementary_filter_ros2");
...
void NodeInterface::initialize(int argc, char * const argv[]) {
    try {
        ...
        std::string NodeName("complementary_filter_ros2");
        SLROSNodePtr = std::make_shared<rclcpp::Node>(NodeName);
        RCLCPP_INFO(SLROSNodePtr->get_logger(),"** Starting the model \"complementary_filter_ros2\" **\n");
		...
		rtmSetErrorStatus(complementary_filter_ros2_M, (NULL));
        complementary_filter_ros2_initialize();
		/* External mode */
		rtSetTFinalForExtMode(&rtmGetTFinal(complementary_filter_ros2_M));
		rtExtModeCheckInit(1);
		{
			boolean_T rtmStopReq = false;
			rtExtModeWaitForStartPkt(complementary_filter_ros2_M->extModeInfo, 1,
				&rtmStopReq);
			if (rtmStopReq) {
				rtmSetStopRequested(complementary_filter_ros2_M, true);
			}
		}
		...
		mSchedulerTimer = SLROSNodePtr->create_wall_timer(std::chrono::nanoseconds(9615385),std::bind(&NodeInterface::schedulerThreadCallback,this));
    }
...
boolean_T NodeInterface::getStopRequestedFlag(void) {
    ...
    return (!(rtmGetErrorStatus(complementary_filter_ros2_M) 
        == (NULL)));
    #else
    return (!(rtmGetErrorStatus(complementary_filter_ros2_M) 
        == (NULL)) || rtmGetStopRequested(complementary_filter_ros2_M));
    #endif
}
...
void NodeInterface::terminate(void) {
    if (mBaseRateThread.get()) {
        ...
        complementary_filter_ros2_terminate();
        rtExtModeShutdown(1);
        mExec.reset();
        SLROSNodePtr.reset();
        rclcpp::shutdown();
...
void NodeInterface::baseRateTask(void) {
  mRunModel = (rtmGetErrorStatus(complementary_filter_ros2_M) ==
              (NULL));
  while (mRunModel) {
    mBaseRateSem.wait();
#ifdef MW_DEBUG_LOG
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModePauseIfNeeded(complementary_filter_ros2_M->extModeInfo, 1,
        &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(complementary_filter_ros2_M, true);
      }
      if (rtmGetStopRequested(complementary_filter_ros2_M) == true) {
        rtmSetErrorStatus(complementary_filter_ros2_M, "Simulation finished");
		mRunModel = false;
        break;
      }
    }
    complementary_filter_ros2_step(
	);
...
void NodeInterface::extmodeBackgroundTask(void)
{
  while (mRunModel) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(complementary_filter_ros2_M->extModeInfo, 1,
                       &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(complementary_filter_ros2_M, true);
      }
