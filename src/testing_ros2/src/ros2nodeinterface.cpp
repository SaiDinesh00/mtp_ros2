...
// C/C++ source code generated on : Sat Jun 15 05:56:42 2024
//
#ifdef _MSC_VER
#pragma warning(push)
...
#include <chrono>
#include <utility>
const std::string SLROSNodeName("testing_ros2");
...
void NodeInterface::initialize(int argc, char * const argv[]) {
    try {
        ...
        std::string NodeName("testing_ros2");
        SLROSNodePtr = std::make_shared<rclcpp::Node>(NodeName);
        RCLCPP_INFO(SLROSNodePtr->get_logger(),"** Starting the model \"testing_ros2\" **\n");
		...
		rtmSetErrorStatus(testing_ros2_M, (NULL));
        testing_ros2_initialize();
		/* External mode */
		rtSetTFinalForExtMode(&rtmGetTFinal(testing_ros2_M));
		rtExtModeCheckInit(1);
		{
			boolean_T rtmStopReq = false;
			rtExtModeWaitForStartPkt(testing_ros2_M->extModeInfo, 1,
				&rtmStopReq);
			if (rtmStopReq) {
				rtmSetStopRequested(testing_ros2_M, true);
			}
		}
...
boolean_T NodeInterface::getStopRequestedFlag(void) {
    ...
    return (!(rtmGetErrorStatus(testing_ros2_M) 
        == (NULL)));
    #else
    return (!(rtmGetErrorStatus(testing_ros2_M) 
        == (NULL)) || rtmGetStopRequested(testing_ros2_M));
    #endif
}
...
void NodeInterface::baseRateTask(void) {
  mRunModel = (rtmGetErrorStatus(testing_ros2_M) ==
              (NULL));
  while (mRunModel) {
    mBaseRateSem.wait();
#ifdef MW_DEBUG_LOG
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModePauseIfNeeded(testing_ros2_M->extModeInfo, 1,
        &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testing_ros2_M, true);
      }
      if (rtmGetStopRequested(testing_ros2_M) == true) {
...
void NodeInterface::extmodeBackgroundTask(void)
{
  while (mRunModel) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(testing_ros2_M->extModeInfo, 1,
                       &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testing_ros2_M, true);
      }
